...
use crate::{
    entity::tasks, error::ApiError, models::AdminWipeBody, queue, roles::UserRole,
...
pub async fn wipe_task_history(
    ...
    let res = tasks::Entity::delete_many().exec(&txn).await.map_err(|e| {
    ...
    let queues = queue::clear_all_agent_queues(&mut redis)
        .await
        .map_err(|e| {
